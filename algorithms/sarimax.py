...
class ForecastSarimax:
    ...
    def run(self):
        ...
        self.view.view_datas("Sarimax")

        self.test_stationarity()

        self.view.autocorrelation("Sarimax")
        self.view.autocorrelation_partial("Sarimax")
        ...
        self.view.view_predicts(predict, self.dataframe_validacao, "Sarimax")

        if self.dataframe_validacao is not None:
            column_value = self.json['column_value']
            self.view.view_predicts_validacao(self.dataframe, self.dataframe_validacao, predict, self.normalizer, None, column_value, "Sarimax")

        self.metrics = self.metrics + self.view.metrics
        self.view_model(model)

        name = f"Sarimax_{datetime.now().strftime("%Y%m%d_%H%M%S")}"
        report = Report()
    ...
    def modeling(self):
        ...
        model = SARIMAX(self.dataframe[column_value].to_numpy(), order=(p, d, q), seasonal_order=(a, b, c, d))
